...
namespace second_api_project
{
    ...
    {    //this is basic authentication
        //this function will recive username and password from client from the requers header 
        public override void OnAuthorization(HttpActionContext actionContext)//action context parameter provid us access to requers and response objrct
        {
            ...
            else//if header is avilable 
            {
                ...
                if(archiveSecurity.login(username,password)) //if the method return true(username and passwordmatched)
                {
